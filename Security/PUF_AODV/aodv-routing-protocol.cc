...
//std::clog stream used for logging to the console
...
bool RoutingProtocol::AodvSpecificPort(Ptr<const Packet> p) {
    UdpHeader udpHeader;
    Ptr<Packet> pCopy = p->Copy(); // Make a copy of the packet for safe extraction

    // Extract the UDP header
    pCopy->RemoveHeader(udpHeader);
    
    // Check if the destination or source port is the AODV port
    return (udpHeader.GetDestinationPort() == AODV_PORT || udpHeader.GetSourcePort() == AODV_PORT);
}
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  std::cout << "checking if node is malicious :if yes, proceed to drop packets" << std::endl;

  Ptr<Node> node = this->GetObject<Node>();
  uint32_t nodeId = node->GetId();
  Ptr<MaliciousNode> maliciousNode = DynamicCast<MaliciousNode>(node);
  if (maliciousNode){
    bool MaliciousFlag = maliciousNode->IsMalicious(); // Retrieve the IsMalicious flag status
      // NS_LOG_UNCOND("Malicious node status: " << (MaliciousFlag ? "Malicious" : "Not Malicious"));
    std::cout<<"Malicious node status: " << MaliciousFlag <<std::endl;
    if (maliciousNode->IsMalicious()) {
      // NS_LOG_UNCOND("Malicious node received RREQ.");
      std::cout << nodeId <<" :Malicious node Flag is set." << std::endl;
      if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER && !AodvSpecificPort(p)) {
          std::cout << "PORT Number calculated"<< std::endl;
          NS_LOG_INFO("Dropping data packet because this node is malicious");
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  // Ptr<Node> node = this->GetObject<Node>(); // Get the node
  // Ptr<MaliciousNode> maliciousNode = DynamicCast<MaliciousNode>(node);

  // if (maliciousNode && maliciousNode->IsMalicious()) {
  //     NS_LOG_INFO("Dropping packet in forwarding because this node is malicious");
  //     return false; // Drop the packet
  // }
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  Ipv4Address originNodeAddress = rreqHeader.GetOrigin();
  // A node ignores all RREQs received from any node in its blacklist

  // Check if the current node is malicious
  Ptr<Node> node = this->GetObject<Node>();
  uint32_t nodeId = node->GetId();
  Ptr<MaliciousNode> maliciousNode = node->GetObject<MaliciousNode>();
  if (maliciousNode){
    bool MaliciousFlag = maliciousNode->IsMalicious(); // Retrieve the IsMalicious flag status
      // NS_LOG_UNCOND("Malicious node status: " << (MaliciousFlag ? "Malicious" : "Not Malicious"));
    std::cout<<"Malicious node status: " << MaliciousFlag <<std::endl;
    if (maliciousNode->IsMalicious()) {
      ...
      // NS_LOG_UNCOND("Malicious node received RREQ.");
      std::cout << nodeId <<" :Malicious node received RREQ." << std::endl;

      RoutingTableEntry toOrigin;
      if (!m_routingTable.LookupRoute(rreqHeader.GetOrigin(), toOrigin)) {
        // No route to origin, create a new one
        Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
        RoutingTableEntry newEntry(dev, rreqHeader.GetOrigin(), true, rreqHeader.GetOriginSeqno(),
                                  m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
                                  rreqHeader.GetHopCount() + 1, src, // next hop is the source of the RREQ
                                  Time((2 * m_netTraversalTime - 2 * rreqHeader.GetHopCount() * m_nodeTraversalTime)));

        m_routingTable.AddRoute(newEntry);
        NS_LOG_UNCOND("Malicious node creates a new route to origin.");
        std::cout << nodeId <<" :Malicious node creates a new route to origin." << std::endl;
        toOrigin = newEntry;
      }

      uint32_t manipulatedHopCount = 0;  // Example manipulation
      uint32_t manipulatedSeqNo = 100;  // Manipulate sequence number
      Time manipulatedLifetime = Seconds(60);  // Manipulated lifetime

      RrepHeader rrepHeader(/*prefix size=*/0, /*hops=*/manipulatedHopCount, /*dst=*/src,
          /*dst seqno=*/manipulatedSeqNo, /*origin=*/originNodeAddress, /*lifetime=*/manipulatedLifetime);
      std::cout <<"Packet manipulated by malicious node "<< nodeId  << std::endl;
      // Create packet and add headers
      Ptr<Packet> packet = Create<Packet>();
      SocketIpTtlTag tag;
      ...
      // Find socket and send packet to the next hop
      Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
      NS_ASSERT(socket);
      socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetDestination(), AODV_PORT));  // Send to the source of RREQ
      return;
    }
    else {
        std::cout << nodeId <<" :Node is not malicious. Normal process continues..." << std::endl;
        // Normal processing of RREQ continues here
    }}
  else {
    std::cout << "maliciousNode is null or uninitialized." << std::endl;
}
  ...
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      ...
      //calculate fitness of the packets recieved and calculate the fitness of each. 
      //Identify the path with least fitness value. 
      //Then lookup for the node from which that packet (the packet with the least fitness value) was received and send the reply to that node
      m_routingTable.LookupRoute (origin, toOrigin);
